...
*/
const WEBCHAT_SETTINGS_NAMESPACE = "dsh-webchat";
...
function apply(ctx, config) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		if (typeof provider?.installSection === "function") {
			provider.installSection(ctx, WEBCHAT_SETTINGS_NAMESPACE, Config, config ?? {}, hooks);
			return;
		}
		import("@deepseek-ai/dsh-settings").then((module) => {
			const legacy = module.installSettingsSection;
			if (typeof legacy === "function") legacy(ctx, WEBCHAT_SETTINGS_NAMESPACE, Config, config ?? {}, hooks);
		});
	});
	sync();
}
